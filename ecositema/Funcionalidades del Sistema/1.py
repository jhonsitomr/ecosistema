...
def simular_dia(especies, alimento_disponible):
    # Ordenar herbívoros y carnívoros por capacidad de búsqueda
    herbivoros = sorted([especie for especie in especies if especie.tipo == 'herbivoro'], key=lambda x: x.capacidad_busqueda, reverse=True)
    carnivoros = sorted([especie for especie in especies if especie.tipo == 'carnivoro'], key=lambda x: (x.capacidad_busqueda * x.tasa_efectividad), reverse=True)

    # Alimentar herbívoros
    for herbivoro in herbivoros:
        alimento_consumido = herbivoro.alimentar(alimento_disponible)
        alimento_disponible -= alimento_consumido

    # Alimentar carnívoros
    for carnivoro in carnivoros:
        alimento_consumido = carnivoro.alimentar(alimento_disponible)
        alimento_disponible -= alimento_consumido
    ...
    # Mortalidad
    for especie in especies:
        if especie.cantidad_individuos <= 0:
            especie.morir()
    ...
    while True:
        ...
        if opcion == '1':
            ...
            for dia in range(dias):
                simular_dia(ecosistema, 100)  # Suponiendo 100 unidades de alimento disponible
